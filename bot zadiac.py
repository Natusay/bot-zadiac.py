...
bot = telebot.TeleBot('5773938390:AAEf_B9CtdCZJvhUsrf_ZTZQe2SCcqcE7Ws')
...
#     text = message.text     # всё что написали переходит в переменную текст. т.е. эти две строки значат пиши привет, и бот отвечает привет и т.д
#     bot.send_message(message.chat.id, text)
#     if message.text == 'Привет, поздоровайся со мной!' or message.text == 'привет':  # общаемся с ботом
#         bot.send_message(message.chat.id, 'И тебе привет кусок мяса!')
       # if message.text == 'Привет' or message.text == 'привет':    # общаемся с ботом
       #  bot.send_message(message.chat.id, 'И тебе привет кусок мяса!')
#     elif: message.text == 'как дела?' or message.text == 'Как дела?'
#         bot.send_message(message.chat.id, 'Как у меня могут быть дела, я тупо бот!!!')
#     else:
#         bot.send_message(message.chat.id, 'Моя твоя не панимает!!!')

@bot.message_handler(commands=['start'])
def start_message(message):
  bot.send_message(message.chat.id,"Привет ✌️, может поздороваешься со мной?!")
...
def get_text_messages(message):
    if message.text == "Привет":
        bot.send_message(message.from_user.id, "Привет, привет, а сейчас я расскажу тебе гороскоп на сегодня.")
